              ...
              <div>
                ...
                <div className="border-t pt-3 flex justify-between items-center">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => handleToggleStatus(emp)} className={`p-2 rounded-lg transition text-white ${isInactive ? 'bg-green-500 hover:bg-green-600' : 'bg-red-500 hover:bg-red-600'}`} title={isInactive ? "Reativar" : "Desligar"}>{isInactive ? <RefreshCw size={18} /> : <div className="flex items-center gap-1 text-xs font-bold"><UserX size={16}/> DESLIGAR</div>}</button>
                  </div>
                </div>
              </div>
            </Card>
...
function PayrollCalculator({ employees, advances, onGenerate, companyData }) {
  ...
  // FILTRO: APENAS FUNCIONÁRIOS ATIVOS
  const activeEmployees = employees.filter(e => e.status !== 'inactive');
        ...
        <div className="overflow-x-auto rounded-xl border border-slate-200">
          <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y divide-slate-100">
              {activeEmployees.map(emp => { const v = getVals(emp); const p = getPending(emp.id); const done = v.advancesIncluded.length > 0; return (<tr key={emp.id} className="hover:bg-slate-50 transition-colors"><td className="p-4"><div className="font-bold text-slate-700">{emp.name}</div><div className="text-xs text-slate-400 mb-1">{emp.role}</div>{p.total > 0 && !done && <div onClick={() => handleApplyAdvances(emp.id, p.total, p.list)} className="cursor-pointer inline-flex items-center gap-1 px-2 py-1 rounded-md bg-orange-100 text-orange-700 text-xs font-bold hover:bg-orange-200 transition"><AlertTriangle size={12}/> Vale: {money(p.total)}</div>} {done && <div className="inline-flex items-center gap-1 px-2 py-1 rounded-md bg-emerald-100 text-emerald-700 text-xs font-bold"><CheckCircle size={12}/> Descontado</div>}</td><td className="p-4"><input type="number" className="w-full p-3 border rounded-lg text-center font-medium focus:ring-2 focus:ring-blue-500 outline-none" placeholder="0" onChange={e => handleInputChange(emp.id, 'days', e.target.value)}/></td><td className="p-4"><input type="number" className="w-full p-3 border border-blue-200 bg-blue-50/50 rounded-lg text-center font-medium focus:ring-2 focus:ring-blue-500 outline-none" placeholder="0" onChange={e => handleOvertimeHoursChange(emp.id, e.target.value, emp)}/></td><td className="p-4"><input type="number" className="w-full p-3 border border-blue-200 bg-blue-50/50 rounded-lg text-right font-bold text-blue-600 focus:ring-2 focus:ring-blue-500 outline-none" value={inputs[emp.id]?.overtime || ''} placeholder="0,00" onChange={e => handleInputChange(emp.id, 'overtime', e.target.value)}/></td><td className="p-4"><input type="number" className="w-full p-3 border border-emerald-200 bg-emerald-50/50 rounded-lg text-right font-bold text-emerald-600 focus:ring-2 focus:ring-emerald-500 outline-none" placeholder="0,00" onChange={e => handleInputChange(emp.id, 'bonus', e.target.value)}/></td><td className="p-4"><input type="number" className="w-full p-3 border border-red-200 bg-red-50/50 rounded-lg text-right font-bold text-red-600 focus:ring-2 focus:ring-red-500 outline-none" value={inputs[emp.id]?.discount || ''} placeholder="0,00" onChange={e => handleInputChange(emp.id, 'discount', e.target.value)}/></td><td className="p-4 text-right bg-blue-50/30"><span className="font-mono font-bold text-lg text-slate-800">{money(v.netTotal)}</span></td></tr>); })}
            </tbody>
          </table>
        </div>
