    ...
    <div className="min-h-screen bg-blue-900 flex items-center justify-center p-4">
      <div className="bg-white p-8 rounded-2xl shadow-2xl w-full max-w-md animate-fade-in">
        <div className="text-center mb-8">
          <div className="bg-blue-100 w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4 text-blue-700"><FileText size={32} /></div>
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          <div><label className="block text-sm font-bold text-slate-700 mb-1">E-mail</label><input type="email" required className="w-full p-3 border rounded-lg focus:ring-2 focus:ring-blue-500" value={email} onChange={e => setEmail(e.target.value)}/></div>
          <div><label className="block text-sm font-bold text-slate-700 mb-1">Senha</label><input type="password" required className="w-full p-3 border rounded-lg focus:ring-2 focus:ring-blue-500" value={password} onChange={e => setPassword(e.target.value)}/></div>
          {error && <div className="text-red-500 text-sm text-center bg-red-50 p-2 rounded">{error}</div>}
          <button type="submit" disabled={loading} className="w-full bg-blue-700 text-white font-bold py-3 rounded-lg hover:bg-blue-800 transition flex justify-center items-center gap-2">{loading ? 'Entrando...' : <><Lock size={18}/> Acessar</>}</button>
        </form>
      </div>
    </div>
  );
...
export default function App() {
  ...
  const [companyData, setCompanyData] = useState(null); // Dados da Empresa
  ...
  useEffect(() => {
    ...
    const unsubSettings = onSnapshot(doc(db, 'users', user.uid, 'settings', 'profile'), (doc) => {
      if (doc.exists()) setCompanyData(doc.data());
    });
      ...
      <header className="bg-blue-700 text-white p-4 shadow-lg print:hidden">
        <div className="w-full px-6 flex justify-between items-center">
          <div className="flex items-center gap-3">
            {companyData?.logoUrl ? (
              <img src={companyData.logoUrl} alt="Logo" className="h-10 w-10 rounded-lg bg-white object-contain p-1"/>
            ) : (
              <FileText className="h-8 w-8" />
            )}
            <h1 className="text-xl font-bold hidden md:block">
              {companyData?.name || 'RH Fácil'}
            </h1>
          </div>
          <nav className="flex gap-4 text-sm items-center">
            <button onClick={() => setView('dashboard')} className={`hover:text-blue-200 ${view === 'dashboard' ? 'font-bold underline' : ''}`}>Início</button>
            <button onClick={() => setView('employees')} className={`hover:text-blue-200 ${view === 'employees' ? 'font-bold underline' : ''}`}>Funcionários</button>
            <button onClick={() => setView('payroll')} className={`hover:text-blue-200 ${view.includes('report') || view === 'payroll' ? 'font-bold underline' : ''}`}>Folha</button>
            <div className="h-4 w-px bg-blue-500 mx-2"></div>
            <button onClick={() => setView('settings')} className="flex items-center gap-1 text-blue-200 hover:text-white transition" title="Configurações da Empresa"><Settings size={18}/></button>
            <button onClick={() => signOut(auth)} className="flex items-center gap-1 text-blue-200 hover:text-white transition"><LogOut size={18}/></button>
          </nav>
        </div>
      </header>
  ...
  return (
    <div className="max-w-2xl mx-auto bg-white p-8 rounded-xl shadow-lg mt-10">
      <h2 className="text-2xl font-bold text-slate-700 mb-6 flex items-center gap-2">
        <Building2 className="text-blue-600"/> Configurações da Empresa
      </h2>
      <p className="text-slate-500 text-sm mb-6">Preencha os dados para que saiam automaticamente nos holerites e relatórios.</p>
      
      <form onSubmit={handleSave} className="space-y-4">
        <div><label className="block text-sm font-bold mb-1">Razão Social / Nome da Empresa</label><input type="text" required className="w-full p-3 border rounded bg-slate-50" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} placeholder="Ex: Padaria do João Ltda"/></div>
        <div className="grid grid-cols-2 gap-4">
          <div><label className="block text-sm font-bold mb-1">CNPJ</label><input type="text" className="w-full p-3 border rounded bg-slate-50" value={formData.cnpj} onChange={e => setFormData({...formData, cnpj: e.target.value})} placeholder="00.000.000/0001-00"/></div>
          <div><label className="block text-sm font-bold mb-1">Telefone</label><input type="text" className="w-full p-3 border rounded bg-slate-50" value={formData.phone} onChange={e => setFormData({...formData, phone: e.target.value})}/></div>
        </div>
        <div><label className="block text-sm font-bold mb-1">Endereço Completo</label><input type="text" className="w-full p-3 border rounded bg-slate-50" value={formData.address} onChange={e => setFormData({...formData, address: e.target.value})}/></div>
        
        <div>
          <label className="block text-sm font-bold mb-1">URL da Logo (Link da Imagem)</label>
          <input type="text" className="w-full p-3 border rounded bg-slate-50" value={formData.logoUrl} onChange={e => setFormData({...formData, logoUrl: e.target.value})} placeholder="https://meusite.com/logo.png"/>
          <p className="text-xs text-slate-400 mt-1">Dica: Copie o link da imagem do perfil do seu Instagram ou Facebook.</p>
        </div>

        {formData.logoUrl && (
          <div className="mt-2 p-2 border rounded bg-slate-50 text-center">
            <p className="text-xs mb-2">Pré-visualização:</p>
            <img src={formData.logoUrl} alt="Logo Preview" className="h-20 mx-auto object-contain"/>
          </div>
        )}

        <div className="pt-4 flex gap-2 justify-end">
          <button type="button" onClick={onSave} className="px-4 py-2 text-slate-500 hover:bg-slate-100 rounded">Cancelar</button>
          <button type="submit" className="px-6 py-2 bg-blue-600 text-white font-bold rounded hover:bg-blue-700">Salvar Configurações</button>
        </div>
      </form>
    </div>
  );
...
// --- DEMAIS COMPONENTES (Dashboard, etc) ---
...
function Dashboard({ changeView, employees, userId }) {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      <div className="text-center py-8"><h2 className="text-3xl font-bold text-slate-700">Painel de Controle</h2><p className="text-slate-500">Visão geral da empresa.</p></div>
...
function PayrollCalculator({ employees, advances, onGenerate, companyData }) {
  ...
  const calculate = () => { if (!dates.start || !dates.end) return alert("Selecione datas."); if(!companyData?.name) alert("Atenção: Configure os dados da empresa nas Configurações para sair no recibo."); const items = employees.map(e => ({ ...e, ...getVals(e), dailyRate: e.type === 'mensalista' ? (e.baseValue/30) : e.baseValue })).filter(i => i.days > 0 || i.netTotal > 0); if (items.length === 0) return alert("Preencha algo."); onGenerate({ startDate: dates.start, endDate: dates.end, items }); };
...
function HoleriteView({ data, onBack, companyData }) {
  ...
  // Dados da empresa que vêm do cadastro
  const empName = companyData?.name || 'EMPRESA (Configure no Menu)';
  const empCnpj = companyData?.cnpj || '00.000.000/0000-00';
  const empLogo = companyData?.logoUrl || null;

  return (
    <div className="max-w-7xl mx-auto pb-20">
      <div className="bg-slate-800 text-white p-4 rounded-xl shadow-lg mb-8 print:hidden flex justify-between">
        <button onClick={onBack} className="flex items-center gap-2 hover:text-blue-300 font-bold"><ArrowLeft size={20} /> Voltar</button>
          ...
          <div key={index} className="bg-white text-black font-sans text-xs mb-10 print:mb-4 print:break-inside-avoid max-w-[21cm] mx-auto border-2 border-black flex">
            <div className="flex-1">
              <div className="border-b border-black p-2 flex justify-between items-end">
                <div className="flex items-center gap-3">
                  {empLogo && <img src={empLogo} alt="Logo" className="h-12 w-12 object-contain" />}
                  <div><h1 className="font-bold text-sm uppercase">{empName}</h1><p className="text-[10px]">CNPJ: {empCnpj}</p></div>
                </div>
                <div className="text-right"><h2 className="font-bold text-sm uppercase">Recibo de Pagamento</h2><p className="text-[10px]">{date(data.startDate)} a {date(data.endDate)}</p></div>
              </div>
