...
import { 
  ...
  DollarSign, Copy, Send, Shield, UserX, RefreshCw, Save
} from 'lucide-react';
...
export default function App() {
  ...
  // --- MEMÓRIA DO CÁLCULO (Estado Elevado para não apagar ao voltar) ---
    ...
    switch(view) {
      ...
      case 'payroll': 
        return <PayrollCalculator 
          ...
          // Passando a memória do App para a calculadora
          inputs={payrollInputs}
          setInputs={setPayrollInputs}
          dates={payrollDates}
          setDates={setPayrollDates}
        />;
...
function CompanySettings({ userId, currentData, onSave, currentUser }) {
  ...
  useEffect(() => {
    if (currentData) setFormData({ name: currentData.name || '', cnpj: currentData.cnpj || '', address: currentData.address || '', phone: currentData.phone || '', logoUrl: currentData.logoUrl || '' });
  }, [currentData]);
    ...
    <div className="max-w-4xl mx-auto animate-fade-in space-y-8">
      <Card>
        ...
        <form onSubmit={handleSaveProfile} className="space-y-4">
          <div><label className="block text-sm font-bold mb-1">Nome da Empresa</label><input type="text" required className="w-full p-3 border rounded-lg bg-slate-50" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})}/></div>
          ...
          <div><label className="block text-sm font-bold mb-1">URL da Logo</label><input type="text" className="w-full p-3 border rounded-lg bg-slate-50" value={formData.logoUrl} onChange={e => setFormData({...formData, logoUrl: e.target.value})}/></div>
      ...
      <Card className="border-l-4 border-l-orange-500">
        ...
        <form onSubmit={handleUpdateSecurity} className="space-y-4 pt-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4"><div><label className="block text-sm font-bold mb-1">Novo E-mail</label><input type="email" className="w-full p-3 border rounded-lg" value={securityData.newEmail} onChange={e => setSecurityData({...securityData, newEmail: e.target.value})} placeholder={currentUser.email} /></div><div><label className="block text-sm font-bold mb-1">Nova Senha</label><input type="password" className="w-full p-3 border rounded-lg" value={securityData.newPassword} onChange={e => setSecurityData({...securityData, newPassword: e.target.value})} /></div></div>
...
function EmployeeManager({ employees, userId }) {
  ...
  const handleToggleStatus = async (employee) => {
    ...
    if (confirm(`Deseja ${isInactive ? 'REATIVAR' : 'DESLIGAR'} ${employee.name}?`)) {
      await updateDoc(doc(db, 'users', userId, 'employees', employee.id), { status: isInactive ? 'active' : 'inactive' });
    }
  };
        ...
        {employees.map(emp => {
          ...
          return (
            <Card key={emp.id} className={`group relative overflow-hidden ${isInactive ? 'bg-slate-100 opacity-80' : ''}`}>
              ...
              <div>
                <div className="flex justify-between items-start mb-2"><div className={`font-bold px-2 py-1 rounded text-xs uppercase tracking-wide ${isInactive ? 'bg-slate-200 text-slate-500' : 'bg-blue-50 text-blue-600'}`}>{emp.role}</div><div className="text-slate-400"><Users size={18}/></div></div>
...
// --- CALCULADORA (CORRIGIDA COM TOTAL E PERSISTÊNCIA) ---
function PayrollCalculator({ employees, advances, onGenerate, companyData, inputs, setInputs, dates, setDates }) {
  // Use as props 'inputs' e 'setInputs' em vez de criar estado local
  ...
  // CORREÇÃO DO TOTAL GERAL: Calcula ao vivo usando os dados atuais
        ...
        <div className="overflow-x-auto rounded-xl border border-slate-200">
          <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y divide-slate-100">
              ...
              <td className="p-4"><input type="number" className="w-full p-3 border rounded-lg text-center font-medium focus:ring-2 focus:ring-blue-500 outline-none" value={inputs[emp.id]?.days || ''} placeholder="0" onChange={e => handleInputChange(emp.id, 'days', e.target.value)}/></td>
              <td className="p-4"><input type="number" className="w-full p-3 border border-blue-200 bg-blue-50/50 rounded-lg text-center font-medium focus:ring-2 focus:ring-blue-500 outline-none" placeholder="0" onChange={e => handleOvertimeHoursChange(emp.id, e.target.value, emp)}/></td>
              <td className="p-4"><input type="number" className="w-full p-3 border border-blue-200 bg-blue-50/50 rounded-lg text-right font-bold text-blue-600 focus:ring-2 focus:ring-blue-500 outline-none" value={inputs[emp.id]?.overtime || ''} placeholder="0,00" onChange={e => handleInputChange(emp.id, 'overtime', e.target.value)}/></td>
              <td className="p-4"><input type="number" className="w-full p-3 border border-emerald-200 bg-emerald-50/50 rounded-lg text-right font-bold text-emerald-600 focus:ring-2 focus:ring-emerald-500 outline-none" value={inputs[emp.id]?.bonus || ''} placeholder="0,00" onChange={e => handleInputChange(emp.id, 'bonus', e.target.value)}/></td>
              <td className="p-4"><input type="number" className="w-full p-3 border border-red-200 bg-red-50/50 rounded-lg text-right font-bold text-red-600 focus:ring-2 focus:ring-red-500 outline-none" value={inputs[emp.id]?.discount || ''} placeholder="0,00" onChange={e => handleInputChange(emp.id, 'discount', e.target.value)}/></td>
              <td className="p-4 text-right bg-blue-50/30"><span className="font-mono font-bold text-lg text-slate-800">{money(v.netTotal)}</span></td></tr>); })}
            </tbody>
          </table>
        </div>
        {/* BARRA DE TOTAL NO RODAPÉ */}
        <div className="fixed bottom-0 left-0 w-full bg-white border-t border-slate-200 p-4 shadow-2xl flex justify-end items-center gap-6 z-50">
          <div className="text-right">
            <span className="block text-xs font-bold text-slate-400 uppercase">Total da Folha</span>
            <span className="text-2xl font-bold text-slate-800">{money(totalPayroll)}</span>
          </div>
          <Button onClick={calculate} className="shadow-xl px-8 py-3 text-lg bg-green-600 hover:bg-green-700">Gerar Documentos <ArrowRight size={20}/></Button>
        </div>
      </Card>
    </div>
  );
